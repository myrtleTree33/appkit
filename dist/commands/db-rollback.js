...
export default (cli) => {
    cli
        ...
        try {
            ...
            logger.info(`Started rolling back the '${opts.target}' database schema to previous version...`);
            await db[opts.target]?.migrate.rollback();
            logger.info(`Started rolling back the '${opts.target}' database schema to previous version... SUCCESS`);
        }
        catch (err) {
            logger.error(err);
            process.exit(1);
